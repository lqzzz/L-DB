...
static Column database_name;//char[16]

static Table sys_table_info;
static Column table_name;//char[16]
...
static Column table_db_name;//char[16]
static Column table_page_solt_count;//int

static Table sys_column_info;
static Column column_num;//int
static Column column_name;//char[16]
static Column column_table_name;//char[16]
static Column column_database;//char[16]
...
static Table sys_index_info;
static FILE* table_info;
static FILE* column_info;
static pthread_mutex_t db_file_mutex = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t table_file_mutex = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t column_file_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static DBnode* read_database_info(void) {
	FILE* fd;
	fd = fopen("DBInfo.df", "a");
	fclose(fd);
...
static void sys_database_init(DBnode* db, char* name, int id, int tablecount, Table* tablehead) {
	db->name_ = name;
	db->id_ = id;
...
static void sys_table_init(Table* table, char* name, 
...
static void sys_column_init(Column* col, char* name, int id, int datatype, int recoffset) {
	col->name_ = name;
...
int database_info_update(DBnode * db){
	...
	fwrite(db->name_, sizeof(char), 16, fd);
	fwrite(&db->table_count, sizeof(int), 16, fd);
...
DBnode* init_sys_data() {
	...
	LIST_ADD_TAIL(&sys_database_info.list_head, &sys_index_info.list_head);
	...
	sys_column_init(&table_auto_increment, "Auto_Increment", 2, INT, sizeof(int) + 16);
	sys_column_init(&table_rec_size, "RecCount", 3, INT, 2 * sizeof(int) + 16);
	sys_column_init(&table_data_len, "DataLen", 4, INT, 3 * sizeof(int) + 16);
	sys_column_init(&table_col_count, "ColumnCount", 5, INT, 4 * sizeof(int) + 16);
	sys_column_init(&table_db_name, "DataBaseName", 6, CHAR, 5 * sizeof(int) + 16);
	...
	sys_column_init(&column_table_name, "TableName", 2, CHAR, sizeof(int) + 16);
